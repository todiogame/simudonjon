...
Les points ci-dessous décrivent des **heuristiques IA / approximations de simulation**,
pas la règle brute de l'objet quand celle-ci laisse un vrai choix au joueur.
- **Sceptre Changeur** — description fournie hors tableur ; **heuristique IA** :
  l'IA choisit en priorité
  un Ange Gardien si elle a une ligne (puissance 8 ou Attrape-Rêves), sinon la plus
  grosse cible absorbable, puis la Fée, puis une cible couverte par une ligne, sinon
  le plus petit monstre estimé. Le monstre choisi est retiré du Donjon et le reste est mélangé.
...
- **Épée vengeresse** / **Dague vengeresse** — **heuristique IA** : l'IA choisit en début de partie la
  meilleure ligne non déjà couverte par le reste de la main, selon le danger cumulé
  du deck (et retombe sur la meilleure ligne globale si tout est déjà couvert).
- **Boule de cristal** — **heuristique IA** : l'annonce est choisie selon un score
  de danger du deck restant (`fréquence × puissance`), pas par une vraie planification.
- **Tempête des âmes** — **heuristique IA** : chaque joueur rend le plus gros monstre
  qu'une ligne passive intacte de sa main couvre déjà, sinon le plus petit possible.
